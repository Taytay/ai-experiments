...
- Source: references/papers/2402.01781/2402.01781.pdf, references/papers/2402.01781/paper.txt

> Provenance: compiled from reading notes taken from the paper (references/papers/2402.01781/notes.md) and the arXiv metadata; the PDF could not be rendered by the Read tool in the pass that produced this file. Re-verify any number against paper.txt before quoting it in the report.
...
- **EVAL-1.** The project's scorer (`scripts/exp_curriculum.py:89-108`) is cloze scoring in this paper's exact sense. The Timmy items, where candidate labels appear in the prompt as demo labels, are hybrid-like. The paper's finding that cloze has the lowest bias but lowest accuracy and "does not reflect a true MCQ setting" matches the observed 20.6 bare-format vs 100.0 trained-format recall gap.
