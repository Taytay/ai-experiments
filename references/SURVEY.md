...
`reports/QUESTIONS.md`. Per-paper summaries with table and figure citations live in
`references/papers/<id>/summary.md`; `references/papers/INDEX.md` lists them by thread. Every number below
...
The four-tier order this survey produced was moved, on 2026-09-14, into the work queue at
`PLAN.md` (repo root), which is the only place the order and per-step status are maintained.
The reasons for the order are the section 1 findings above, referenced by ID. The three
corrections the survey made to `reports/QUESTIONS.md` (LIT-1 paraphrase count, the Qwen2-0.5B
baseline in MODEL-2, the bioS-couple footing for DATA-1) have been applied in that file.
