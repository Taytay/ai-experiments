...
Items are any set in REAL-6's format (REAL-6 itself, the novel merchants); the user histories come from REAL-6.
"""
import math
...
def usage_prior(user):
    u = _users()[user]; c = Counter(h["label"] for h in u["history"]); tot = sum(c.values()) + len(u["categories"])
...
def lookup(item):
    ...
    u = _users()[item["user"]]; labs = Counter(h["label"] for h in u["history"] if h["merchant"] == item["merchant"])
    if not labs:
        return None
...
def scorecard(recs, items, n_boot=500, seed=0):
    """recs: {id: per-item record with sum_lp, answer}; items: {id: item}. Returns a dict of metrics, baselines and user intervals."""
    ...
    per = {}
    for i in ids:
        s = _scores(recs[i]); p = prob[i]; k = len(s); r = _rank(s, y[i]); pr = usage_prior(users[i])
        lk, ou = lookup(items[i]), other_users(items[i])
