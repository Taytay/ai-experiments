...
| 26 | Sequence packing for the knowledge stream via unsloth's `SFTTrainer` (`packing=True`, padding-free): verify block-diagonal attention and per-sequence positions on a packed batch, then arm A packed vs unpacked at the same token budget and seed, within the section 9.2 floor on the frozen ladder; report tokens/s, MFU and loss-bearing-token fractions. Belongs right after step 9 in priority (same mixture-by-tokens change); use it for steps 10, 15 and 17 afterwards. | TRAIN-6 | 9 | doing | |
