...
`report/REPORT.md`, `report/improvements.html` and `experiments/*.py`, same date.

Suggested order of attack: STAT-2 and STAT-3 first (save per-item scores, freeze item sets),
because every other comparison depends on them; then EVAL-1/EVAL-2/EVAL-3 (scoring validity),
MODEL-4 (tokenizer confound), STAT-1 (seeds); then the new arms (MODEL-1 7B, MODEL-2 encoder MLM,
BASE-1 RAFT).

**Update 2026-09-14, after the literature survey.** `report/SURVEY.md` (34 papers, per-paper
summaries under `docs/papers/`) re-prioritizes this register in four tiers and adds one item,
TRAIN-5 (masked fine-tuning of the decoder). Its tier 1 differs from the order above: after the
free scorer and persistence fixes, the highest-yield runs are BASE-3 soft-label prompt
distillation, TRAIN-5 masked fine-tuning, and the TRAIN-1 loss fix (all-answer loss plus a
self-teaching stream), ahead of the 7B and encoder arms. See SURVEY.md section 2.
...
`experiments/bench_throughput.py` (forward-only scoring, LoRA bf16 at 64 and 512 tokens, QLoRA
...
(`experiments/exp_curriculum.py:89-108`, `exp_knowledge_injection.py:58-78`). The argmax option is
...
**Status (2026-09-14):** survey completed; 34 papers read and summarized under `docs/papers/`,
synthesis and re-prioritization in `report/SURVEY.md`. Semantic Scholar and the arXiv API were
...
**Answer.** Not considered so far; nothing in `experiments/` uses a masked-LM head. It fits this
...
(54%)"; the caveat appears only in 6.2 and the HTML.
