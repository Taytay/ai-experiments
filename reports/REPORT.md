...
The two section 6 adapters saved by plain peft failed to load in the first pass (the same loader issue as 9.4); they are queued after the running jobs and their rows are added to `reports/gen_Qwen2.5-3B.md` when done. Merchant items for fine-tuned models need the section 4 recipes re-run with saved adapters, which is its own step.
