...
# Steps from row 47 (REPORT.md 49): the no-DB arm at 800 (it was under-trained at 200), the record arm at 200 (at REAL-6's ceiling
# there, section 48). Plain folds first for both arms, then the rename-augmented ones.
cd "$(dirname "$0")/../.." || exit 1
mkdir -p logs
AD=models/adapters; CAT=categoriser_Qwen2.5-3B-Instruct; G=logs/gpu42
export LOAD_4BIT=1
...
for ren in 0 0.5; do
  for f in 0 1 2 3; do
    for db in none ret; do
      ...
      ST=$([ $db = none ] && echo 800 || echo 200); TAG=$([ $ST = 200 ] && echo "" || echo "_st$ST")
