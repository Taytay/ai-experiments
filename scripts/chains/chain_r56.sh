...
#   P   plain recipe, 800 steps, bf16 base, seed 0                -> precision at the row 47 recipe (4-bit: 73.3 +- 2.3); seeds 1, 2 and the
#       record arm follow in chain_r56b.sh after row 42 resumes
...
[ -n "$SPECS" ] || run S_smoke_alllab env SMOKE=1 LOAD_4BIT=1 ALL_LABELS=1 uv run python scripts/exp_categoriser.py llm none  # first GPU use of both paths
[ -n "$SPECS" ] || run S_smoke_bf16 env SMOKE=1 LOAD_4BIT=0 RUN_TAG=bf16 uv run python scripts/exp_categoriser.py llm none

# spec: name : steps : all_labels : load_in_4bit : run_tag : seed
for spec in ${SPECS:-bf16plain:200:0:0:bf16:0 bf16st800:800:0:0:st800_bf16_s0:0 alllab100:100:1:1:st100:0 alllab200:200:1:1::0 alllab400:400:1:1:st400:0 bf16alllab:200:1:0:bf16:0}; do
  IFS=: read -r name steps al q4 tag seed <<< "$spec"
  A=${CAT}_none${tag:+_$tag}$([ $al = 1 ] && echo _alllab)_lora
  [ -d $AD/$A ] || run M_train_$name env SEED=$seed LOAD_4BIT=$q4 ALL_LABELS=$al STEPS=$steps RUN_TAG=$tag uv run python scripts/exp_categoriser.py llm none
...
echo "CHAIN DONE${SPECS:+ (part 2)} $(date +%H:%M)" >> $G.log
